            ...
            for output in self.config.task.output:
                if hasattr(self, f"{stage}_{output}_{m}"):
                    try:
                        ...
                        pred = y_hat[output]
                        target = y[output]
                        #print('Entered model/base.py/log_metrics/try')
                        #print('pred', pred) # pred.cpu().numpy())
                        #print('target', target) # target.cpu().numpy())
...
class BenchMarkModel(BaseModel):
    ...
    def _do_step(
        ...
        if stage == "test":
            prediction_file_name = "/zhome/c3/9/126583/DL_project/Project/DL_project/prediction_labels/prediction.txt"   
            target_file_name = "/zhome/c3/9/126583/DL_project/Project/DL_project/prediction_labels/target.txt"
            
            # Target
            #y_mod = self.get_labels(unbatch(batch))
            #print(y_mod)
            print('NOW PRINTING NODE LABELS')
            tensor_of_interest = y['node_label']
            output_of_interest = unbatch(tensor_of_interest, batch.batch)

            for protein in output_of_interest:
                the_protein = protein.cpu()
                #print(the_protein.numpy())
                with open(target_file_name, 'a') as file:
                    file.write('#NewArray\n')
                    np.savetxt(file, the_protein.numpy(), fmt = '%d', delimiter = '\t')
            
            #print(output_of_interest)
            #print(tensor_data)
            #/zhome/c3/9/126583/DL_project/Project/DL_project/prediction_labels/prediction.txt
            #/zhome/c3/9/126583/DL_project/Project/DL_project/prediction_labels/target.txt
        
            # Prediction
            tensor_of_interest2 = y_hat['node_label']
            output_of_interest2 = unbatch(tensor_of_interest2, batch.batch)
            for protein in output_of_interest2:
                the_protein2 = protein.cpu()
                ##print(the_protein2.detach().numpy())
                with open(prediction_file_name, 'a') as file:
                    file.write('#NewArray\n')
                    np.savetxt(file, the_protein2.detach().numpy(), fmt = '%f', delimiter = '\t')
